...
parser.add_argument('--is_training', type=int, default=0, help='training or testing')
parser.add_argument('--model_id', type=str, default='HyperIMTS', help='model id')
parser.add_argument('--model_name', type=str, default='HyperIMTS', help='model name')
parser.add_argument('--checkpoints', type=str, default='storage/results/', help='where to save model checkpoints in training')

# dataset & data loader
parser.add_argument('--dataset_name', type=str, default='P12', help='dataset type')
parser.add_argument('--dataset_root_path', type=str, default='storage/datasets/P12/', help='root path of the data file')
...
parser.add_argument('--label_len', type=int, default=300, help='start token length. Should be zero for tasks other than forecasting')
parser.add_argument('--pred_len', type=int, default=300, help='prediction sequence length. Should be zero for tasks other than forecasting')
...
parser.add_argument('--gpu_id', type=int, default=1, help='primary gpu id, will be overwritten when use_multi_gpu is 1. Originally named as --gpu.')
parser.add_argument('--use_multi_gpu', type=int, help='use multiple gpus, via huggingface accelerate library', default=1)
parser.add_argument('--gpu_ids', type=str, default="0,1,2,3", help='string of device ids for multile gpus. Originally named as --devices.')
...
# adaptive test related arguments
parser.add_argument('--enable_', type=int, default=1, help='Enable test-time adaptation (1: True, 0: False)')
parser.add_argument('--enable_adaptation', type=int, default=1, help='Enable adaptation during  (1: True, 0: False)')
parser.add_argument('--uncertainty_threshold', type=float, default=0.005, help='Threshold for uncertainty-based sample selection')
parser.add_argument('--adapt_lr', type=float, default=1e-3, help='Learning rate for adaptation during ')
parser.add_argument('--ue_adapt_lr', type=float, default=1e-4, help='Learning rate for uncertainty estimation network during ')
parser.add_argument('--cali_hidden_dim', type=int, default=64, help='Hidden dimension for calibration network')
parser.add_argument('--cali_var_wise', type=int, default=1, help='Whether to use variable-wise calibration (1: True, 0: False)')
parser.add_argument('--uncertainty_hidden_dim', type=int, default=128, help='Hidden dimension for uncertainty estimation network')

# uncertainty
parser.add_argument('--enable_uncertainty_training', type=int, default=1, help='Enable uncertainty estimation network training (1: True, 0: False)')

parser.add_argument('--_ema_alpha_trigger', type=float, default=0.25, help='EMA alpha for uncertainty estimation network during ')
parser.add_argument('--_std_k_trigger', type=float, default=0.75, help='Std k for uncertainty estimation network during ')
parser.add_argument('--_ema_alpha_alloc', type=float, default=0.75, help='EMA alpha for sample selection during ')
parser.add_argument('--_std_k_alloc', type=float, default=0.25, help='Std k for sample selection during ')

# model configs
# common
parser.add_argument('--patch_len', type=int, default=6, help='patch length. Also used as period_len in some models (SparseTSF).')
parser.add_argument('--patch_stride', type=int, default=6, help='stride when splitting patches. Originally named as --stride.')
...
parser.add_argument('--enc_in', type=int, default=12, help='encoder input size / input time series number of variables. In most cases, it should be adjusted per dataset') 
parser.add_argument('--dec_in', type=int, default=12, help='decoder input size. Usually it is the same as --enc_in')
parser.add_argument('--c_out', type=int, default=12, help='output size. Usually it is the same as --enc_in')
