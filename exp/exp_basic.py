...
class Exp_Basic(object):
    ...
    def _acquire_device(self):
        """Acquire the runtime device based on configurations.
        use_multi_gpu=True: Prefer the device managed by accelerate; if gpu_ids are provided, set CUDA_VISIBLE_DEVICES and let each process select its local device.
        use_multi_gpu=False: Use the single GPU specified by gpu_id.
        ...
        if self.configs.use_gpu:
            if self.configs.use_multi_gpu:
                ...
                if self.configs.gpu_ids is not None:
                    ...
                    self.configs.gpu_id = device_ids[1]  
