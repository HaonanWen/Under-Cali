...
class Model(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        if exp_stage in ["train", "val"]:
            ...
            # print(f"Pred shape after unpadding and reshaping: {pred.shape}")
    ...
    def unpad_and_reshape(
        ...
        """
        将展平的张量根据掩码和原始形状还原为未填充的张量。

        Args:
            tensor_flattened (Tensor): 展平的张量，形状为 (batch_size, num_observations)。
            original_mask (Tensor): 原始掩码，形状为 (batch_size, seq_len, enc_in)。
            original_shape (Tuple): 目标张量的形状，形状为 (batch_size, seq_len, enc_in)。

        Returns:
            Tensor: 还原后的未填充张量，形状为 original_shape。
        """
        ...
        for i in range(batch_size):
            masked_indices = original_mask[i].view(-1).nonzero(as_tuple=True)[0]  # 获取非填充位置的索引
            unpadded_sequence = tensor_flattened[i][:len(masked_indices)]   # 提取未填充的序列
            result[i].view(-1)[masked_indices] = unpadded_sequence   # 将未填充的序列放回原始位置
