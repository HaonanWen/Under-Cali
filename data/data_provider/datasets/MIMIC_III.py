...
class Data(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Split into 20% train, 5% val, 75% test in temporal order (no shuffling)
        ...
        # First split: 75% test, 25% train+val
        sample_index_train_val, sample_index_test = train_test_split(sample_index_all, test_size=0.75, shuffle=False)
        # Second split: 20% of train_val (i.e., 5% of all) for val, 80% of train_val (i.e., 20% of all) for train
        sample_index_train, sample_index_val = train_test_split(sample_index_train_val, test_size=0.2, shuffle=False)
    ...
    def preprocess(self):
        ...
        #     )
            ...
            # Align dataset with custom temporal split indices (20/5/75)
            dataset_all = ConcatDataset([
                task.get_dataset((0, "train")), 
                task.get_dataset((0, "val")), 
                task.get_dataset((0, "test")), 
            ])
            # Create subset based on precomputed sample_index for the chosen split
            dataset = torch.utils.data.Subset(dataset_all, self.sample_index.tolist())
