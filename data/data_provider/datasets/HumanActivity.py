...
class Data(Dataset):
    ...
    def preprocess(self):
        ...
        # First: keep the first 25% as "seen" (train+val), last 75% as test
        seen_data, test_data = model_selection.train_test_split(
            human_activity, train_size=0.25, random_state=42, shuffle=False
        )
        # Then: split seen into train:val = 80%:20% -> 0.2 and 0.05 of the whole
        train_data, val_data = model_selection.train_test_split(
            seen_data, train_size=0.8, random_state=42, shuffle=False
        )
        ...
        if self.flag != "val":
            ...
            if self.configs.collate_fn == "collate_fn_patch":  # 计算patch级别的最大长度
                n_patch: int = SEQ_LEN // PATCH_LEN
                n_patch_y: int = math.ceil(self.configs.pred_len / PATCH_LEN)
