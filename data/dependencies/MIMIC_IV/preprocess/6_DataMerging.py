...
merged_df1=pd.concat([inputs_df, lab_df], ignore_index=True)
# merged_df2=(merged_df1.append(outputs_df)).reset_index()
merged_df2=pd.concat([merged_df1, outputs_df], ignore_index=True)
# merged_df2.drop(columns="level_0",inplace=True)
if "level_0" in merged_df2.columns:
    merged_df2.drop(columns="level_0", inplace=True)
# merged_df=(merged_df2.append(presc_df)).reset_index()
merged_df=pd.concat([merged_df2, presc_df], ignore_index=True)
